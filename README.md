...
```
![Статистика RFM](https://drive.google.com/uc?export=view&id=1Um70g41tWmkVtISj4DhvoT-x0TKjSTyP)

**7. Список клиентов по сегментам**
```sql
spisok_card as 
(
select
...
```
![RFM распределение](https://drive.google.com/uc?export=view&id=113IQgw1k5CUmxXC5QFvT6BVaZZOeTTg5)

## Выводы

- Клиентская база распределена по 27 сегментам, при этом крупнейшие группы — `111` (VIP) и `333` (потерянные клиенты), каждая ~13%.

- Сегмент `111` — ключевое ядро бизнеса: самые активные, частые и прибыльные клиенты, требующие приоритетного удержания.

- Сегмент `311` — уходящие VIP: небольшая доля, но высокий риск потери значимой выручки, необходима персональная реактивация.

- Сегмент `131` — потенциальные VIP: клиенты с высоким чеком, но низкой частотой, основной потенциал роста.

- Сегмент `113` — новые активные клиенты: перспективная группа для увеличения среднего чека и формирования лояльности.

- Сегмент `333` — наименее ценные клиенты: низкий чек, редкие покупки, низкий приоритет для маркетинга.

- Высокая доля неактивных сегментов (`233`, `332`) указывает на риск оттока и необходимость сегментированных стратегий возврата.

- Ключевой риск — снижение активности и потеря ценных клиентов, при этом потенциал роста сосредоточен в средних и новых сегментах.
